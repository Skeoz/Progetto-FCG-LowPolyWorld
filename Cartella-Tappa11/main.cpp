...
// AABB: Axis-Aligned Bounding Box (La scatola che contiene un pezzo di mappa)
struct AABB {
    glm::vec3 minP;
    glm::vec3 maxP;
};
...
// Piano matematico per il Frustum
struct Plane {
    glm::vec3 normal;
    float distance;
};
...
// Estrae i 6 piani visivi dalla matrice di proiezione*vista
Frustum extractFrustum(const glm::mat4& vp) {
    Frustum f;
    // Sinistra, Destra, Basso, Alto, Vicino, Lontano
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 2; ++j) {
...
// Controlla se la scatola AABB interseca il Frustum
bool isAABBVisible(const AABB& aabb, const Frustum& f) {
    for (int i = 0; i < 6; ++i) {
        ...
        if (glm::dot(f.planes[i].normal, p) + f.planes[i].distance < 0.0f) {
            return false; // Completamente fuori da questo piano visivo
        }
    }
...
float cameraSpeed = 0.5f; float yawSensitivity = 0.1f; float pitchSensitivity = 0.08f;

// --- PARSER OBJ ---
bool loadOBJ(const std::string& path, std::vector<Vertex>& out_vertices) {
    // [STESSO CODICE DI LOAD_OBJ DELLA TAPPA 10]
...
// ==========================================
// SHADER TERRENO, SKYBOX E MODELLI
// ==========================================
...
int main() {
    ...
    // 2. CREAZIONE DEI CHUNK (La magia della Tappa 11)
    std::vector<Chunk> terrainChunks;
    int CHUNK_SIZE = 64; // Dimensione in vertici di ogni tassello
    ...
    for (int startY = 0; startY < rows - 1; startY += CHUNK_SIZE - 1) {
        for (int startX = 0; startX < cols - 1; startX += CHUNK_SIZE - 1) {
            ...
            glBindBuffer(GL_ARRAY_BUFFER, chunk.VBO); glBufferData(GL_ARRAY_BUFFER, chunkVertices.size() * sizeof(Vertex), chunkVertices.data(), GL_STATIC_DRAW);
    ...
    glm::vec3 housePos = gridPositions[(int)(rows * 0.60f)][(int)(cols * 0.55f)]; housePos.z += 0.001f;
    ...
    while (window.isOpen()) {
        ...
        float bivAct = glm::smoothstep(0.1f, -0.1f, sH); glm::vec3 bivColor = glm::vec3(1.0, 0.55, 0.1) * bivAct * 2.0f;
        ...
        glm::mat4 viewProj = projection * view; // Fondamentale per il Culling!
        ...
        // Qui avviene la magia: cicliamo i Chunk e disegniamo SOLO quelli visibili
        ...
        // Output in console ogni secondo per vedere l'ottimizzazione in azione
        ...
        if (!houseLoadedVertices.empty()) {
            ...
            hModel = glm::rotate(hModel, glm::radians(90.0f), glm::vec3(1, 0, 0)); hModel = glm::scale(hModel, glm::vec3(0.009f)); 
