...
void main() {
    ...
    // Attenuazione luce ricalibrata per distanze chilometriche (coeff. quadratico 0.0005)
...
int main() {
    ...
    // VSYNC ATTIVO: fondamentale per stabilizzare il framerate e permettere alla GPU di gestire meglio il carico di rendering 
    ...
    while (window.isOpen()) {
        ...
        // ==========================================
        // 1. GESTIONE COLLISIONI SOLIDE - BIVACCO (TAPPA 15 FIX)
        // Calcolo di push-back per evitare di superare i perimetri del modello solido.
        // ==========================================
        float playerRadius = 2.0f; // Skin collisore telecamera
        AABB& box = houseCollisionAABB;

        // Verifichiamo se il giocatore è entrato nel volume solido (XY + skin, e altezza Z)
        if (cameraPos.x >= box.minP.x - playerRadius && cameraPos.x <= box.maxP.x + playerRadius &&
            ...
            // Applichiamo il Push-Back laterale: il motore rigetta fuori la camera.
            ...
            // (La componente Z è gestita separatamente dal ground clamping qui sotto)
        }
        ...
        float verticalOffset = 10.0f; // Bivacco
        ...
        // Matrici di proiezione. Far plane settato a ben 8000.0f per dominare la scala a 5km.
        glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float)window.getSize().x / window.getSize().y, 0.1f, 8000.0f);
        ...
        // Iniezione uniform per la nebbia atmosferica
        ...
        if (!houseLoadedVertices.empty()) {
            ...
            // Traslazione ancorata al vertical offset e modello a 15.0f
            glm::mat4 hModel = glm::translate(glm::mat4(1.0f), housePos + glm::vec3(0, 0, verticalOffset));
            hModel = glm::rotate(hModel, glm::radians(90.0f), glm::vec3(1, 0, 0)); hModel = glm::scale(hModel, glm::vec3(15.0f)); 
        ...
        // ==========================================
        // 6. HUD AMBRA 
        // Nuova riga aggiunta per tenere sotto controllo lo stato del ciclo temporale.
        // ==========================================
        ...
        // Indicatore dinamico del timer solare controllato col tasto P
        ...
        // Impilamento vettoriale delle 4 stringhe telemetriche
