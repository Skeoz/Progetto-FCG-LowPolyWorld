...
I parametri di volo sono stati tarati per superare le distanze continentali:
...
**Soluzione:**
Il moltiplicatore Z delle `gridPositions` è stato raddoppiato e impostato a **0.5f**, restituendo creste affilate, valli vertiginose e costoni alpini maestosi. Per preservare l'equilibrio cromatico della vegetazione e della roccia grigia nuda, i limiti di transizione delle funzioni `smoothstep` nel Fragment Shader del terreno sono stati moltiplicati per un fattore di scala di 2.5, stabilizzando la nuova *snowline* tra i 625 e i 1500 metri virtuali di quota:

vec3 tC = (FragPos.z < 625.0) ? mix(cV, cR, smoothstep(-50.0, 875.0, FragPos.z)) : mix(cR, cS, smoothstep(875.0, 1500.0, FragPos.z));
...
**Soluzione:**
È stata stabilita una densità della nebbia perfettamente simmetrica su entrambi i Fragment Shader, impostando `fogDensity = 0.00015`. Questo valore garantisce un'atmosfera tersa da alta montagna, permettendo di scorgere l'orizzonte profondo e facendo fondere sia la montagna che la casa in modo omogeneo nel cielo man mano che ci si allontana.
...
* **Dimensione:** Il modello è stato scalato a **22.0f** per mantenere una visibilità ottimale nello spazio immenso senza sprofondare nel terreno.
* **Potenza Luce:** Il moltiplicatore di intensità (`bivColor`) è stato spinto a **10.0f** per generare un flusso energetico arancione marcato.
* **Equazione di Attenuazione:** I coefficienti lineare e quadratico nel Fragment Shader del terreno sono stati drasticamente ridotti per consentire ai fotoni di viaggiare su distanze chilometriche prima di estinguersi:

float att = 1.0 / (1.0 + 0.002 * distToLight + 0.00005 * (distToLight * distToLight));

La lampadina fisica (`wLightPos`) è stata sollevata a quota `verticalOffset + 28.0f` per centrarsi perfettamente sotto il soffitto rialzato del modello ingrandito.

### 4. Ristrutturazione della Fisica Tridimensionale
L'allargamento del bivacco a scala 22.0f ha reso la vecchia scatola di collisione AABB totalmente inutile, poiché copriva solo il centro della stanza, permettendo alla telecamera di trapassare i muri perimetrali esterni.

**Soluzione:**
L'impronta a terra della scatola di collisione tridimensionale è stata ampliata a un raggio di +/-24 unità e il tetto è stato riproporzionato a quota 55.0f. La quota inferiore è ancorata a -10.0f sotto lo zero per evitare che il *Ground Clamping* (che agisce ad altezza occhi 1.78f) bypassasse i controlli fisici a livello di fondovalle:

houseCollisionAABB.minP = housePos + glm::vec3(-24.0f, -24.0f, -10.0f);
houseCollisionAABB.maxP = housePos + glm::vec3( 24.0f,  24.0f,  55.0f);

### 5. Ottimizzazione Computazionale (VSync) e Interfaccia Telemetrica
L'assenza di un limite di framerate forzava la GPU a elaborare frame ridondanti, generando un carico termico ingiustificato. Inoltre, l'interazione con il ciclo giorno/notte mancava di feedback visivo.
...
[Inizializzazione] -> Configurazione Griglia DEM a 5000.0f con asse Z potenziato a 0.8f
                    -> Generazione barriera fisica AABB Bivacco estesa (Impronta 24x24, Tetto 55)
                          │
                          ▼
[Game Loop]         -> Input WASD spinto a velocità Jet (300.0f)
