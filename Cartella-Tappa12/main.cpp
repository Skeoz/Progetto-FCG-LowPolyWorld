...
glm::vec3 cameraPos   = glm::vec3(0.0f, -400.0f, 200.0f);
...
// VELOCITÀ BILANCIATA: 50 unità al secondo
...
int main() {
    ...
    auto getTerrainHeight = [&](float worldX, float worldY) -> float {
        ...
        int x0 = (int)gridX; int x1 = std::min(x0 + 1, cols - 1);
        int y0 = (int)gridY; int y1 = std::min(y0 + 1, rows - 1);
    ...
    while (window.isOpen()) {
        ...
        // OFFSET VERTICALE ABBASSATO: 8.0f per appoggiare il pavimento del bivacco sulla neve
