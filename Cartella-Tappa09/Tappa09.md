...
Uno Skybox deve comportarsi come il cielo reale: non si deve mai poter raggiungere il suo bordo traslando all'interno della scena. Nella prima implementazione, lo Skybox veniva disegnato come un normale cubo fisso nella scena, e muovendo la telecamera ci si poteva "scontrare" con le sue facce, rompendo l'illusione atmosferica.

### Analisi e Soluzione
Per creare l'illusione di infinità, si è applicato un trucco algebrico nella View Matrix dello Skybox. Prima di passare la matrice della telecamera allo shader, si è creata una versione  rimuovendo la componente di traslazione (la parte posizionale). 
...
```
In questo modo, lo Skybox viene disegnato sempre centrato perfettamente sulla telecamera. Quando il giocatore si muove con WASD, la telecamera trasla, ma lo Skybox "lo insegue" istantaneamente, facendo percepire che le montagne si muovono mentre il cielo e le stelle rimangono fissi all'infinito.
...
```
Questa tecnica ha permesso di ottenere un sole sferico e accecante, circondato da un'atmosfera calda che si infiamma  al tramonto, perfettamente sincronizzato con le luci e le ombre proiettate sul ghiacciaio sottostante.

---

## Problematica 3: Integrazione del Cielo Stellato Procedurale (Night Noise)
In una prima implementazione, il cielo notturno risultava semplicemente un blu scuro piatto. L'approccio procedurale (senza texture) rendeva difficile la generazione di centinaia di punti luce "casuali" ma fissi nello spazio.
...
Si è poi legata la visibilità di queste stelle alla coordinata verticale del sole, in modo che appaiano gradualmente solo durante l'ora blu/notte alpina e si spengano durante l'alba, garantendo una transizione realistica tra i biomi temporali.
